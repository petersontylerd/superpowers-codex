...
**Input:** User pastes 5–20 records with columns: Parent Issue, Sub-Issue, Client Value, Definition of Done (optionally including Parent Issue ID / Sub-Issue ID).
...
**Input:** User provides a table path (CSV / Markdown / JSON) and references a specific record (row number, `Sub-Issue ID`, or a stable key like `Parent Issue ID` + `Sub-Issue`).
...
**Input:** User provides a JSON file path and says “update row 12” or “update Sub-Issue ID=PI-4-SI-2”, but the JSON could be:
