...
description: Use when grooming backlog items where descriptions or Definitions of Done are vague or not deliverable-anchored, especially for Parent/Sub-Issue record sets or when updating specific rows in CSV/Markdown/JSON backlog files.
---
...
Expected columns: `Parent Issue`, `Sub-Issue`, `Description`, `Definition of Done`.
...
**Table update mode (edit in-place)** if the user provides a backlog file path (CSV / Markdown table / JSON) *and* references specific record(s) (row number, `id`, or key like `Parent Issue` + `Sub-Issue`).
...
- Confirm which fields to refine (default: all four columns, including `Parent Issue`).
...
- **Description:** 1–3 sentences; what/why; no implementation trivia; no scope creep.
...
- Description: “Capture the metrics and terminal fields PI teams use today when Cardiology LOS anomalies occur, per site, including calculation notes and thresholds.”
- Definition of Done:
...
- Prefer selecting rows by `id` or row number; accept `Parent Issue` + `Sub-Issue` only if it matches uniquely.
- Preserve the file’s existing formatting; avoid re-wrapping/re-ordering unrelated content.
- For JSON: confirm which shape you’re editing (array vs keyed object vs nested) before interpreting “row N” or `id=...`. See `json-shapes.md`.
